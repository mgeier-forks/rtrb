...
unsafe impl<{{ params }}> Sync for RingBuffer<{{ args }}> where T: Send {}

// NB: `Send` might be implemented by different storage backends,
// but it is not necessary for correct behavior of the RingBuffer.

impl<{{ params }}> PartialEq for RingBuffer<{{ args }}> {
    fn eq(&self, other: &Self) -> bool {
        core::ptr::eq(self, other)
    }
}

impl<{{ params }}> Eq for RingBuffer<{{ args }}> {}
...
{{ doctest_create_ring_buffer(64) }}
    ...
    /// ```
    pub fn producer(&self) -> Option<Producer<{{ tick_blank }}>> {
...
{{ doctest_create_ring_buffer(64) }}
    ...
    /// assert_eq!(c.pop(), Ok(20));
    /// ```
    pub fn consumer(&self) -> Option<Consumer<{{ tick_blank }}>> {
