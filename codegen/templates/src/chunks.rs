...
pub struct WriteChunkUninit<'a, T{{ N_param }}> {
    ptr: *mut T,
    len: usize,
    producer: &'a Producer<{{ arc_tick_a }}T{{ N_arg }}>,
}

impl<'a, T{{ N_param }}> WriteChunkUninit<'a, T{{ N_arg }}> {
    pub(super) unsafe fn new(producer: &'a Producer<T{{ N_arg }}>, n: usize, offset: usize) -> Self {
        Self {
...
impl<'a, T: Default{{ N_param }}> From<WriteChunkUninit<'a, T{{ N_arg }}>> for WriteChunk<'a, T{{ N_arg }}> {
    /// Fills all slots with the [`Default`] value.
    fn from(chunk: WriteChunkUninit<'a, T{{ N_arg }}>) -> Self {
...
#[derive(Debug, PartialEq, Eq)]
pub struct WriteChunk<'a, T{{ N_param }}>(Option<WriteChunkUninit<'a, T{{ N_arg }}>>);

impl<T{{ N_param }}> Drop for WriteChunk<'_, T{{ N_arg }}> {
    fn drop(&mut self) {
...
{% if contiguous %}
#[derive(Debug, PartialEq, Eq)]
pub struct ReadChunk<'a, T{{ N_param }}> {
    ptr: *mut T,
    len: usize,
    consumer: &'a Consumer<{{ arc_tick_a }}T{{ N_arg }}>,
}

impl<'a, T{{ N_param }}> ReadChunk<'a, T{{ N_arg }}> {
    pub(super) unsafe fn new(consumer: &'a Consumer<T{{ N_arg }}>, n: usize, offset: usize) -> Self {
        Self {
            // SAFETY: Caller must guarantee that `offset` is valid.
...
unsafe impl<T: Send{{ N_param }}> Send for WriteChunkUninit<'_, T{{ N_arg }}> {}
...
// It is therefore safe to move it to another thread.
unsafe impl<T: Send{{ N_param }}> Send for ReadChunk<'_, T{{ N_arg }}> {}

impl<T{{ N_param }}> WriteChunkUninit<'_, T{{ N_arg }}> {
{% if contiguous %}
...
impl<T{{ N_param }}> WriteChunk<'_, T{{ N_arg }}> {
{% if contiguous %}
    /// Returns a slice for writing to the requested slots.
    ///
...
impl<T{{ N_param }}> ReadChunk<'_, T{{ N_arg }}> {
{% if contiguous %}
...
impl<'a, T{{ N_param }}> IntoIterator for ReadChunk<'a, T{{ N_arg }}> {
    type Item = T;
    type IntoIter = ReadChunkIntoIter<'a, T{{ N_arg }}>;
...
/// Non-iterated items remain in the ring buffer.
#[derive(Debug, PartialEq, Eq)]
pub struct ReadChunkIntoIter<'a, T{{ N_param }}> {
    chunk: ReadChunk<'a, T{{ N_arg }}>,
    iterated: usize,
}

// TODO: take "skip" into account?
impl<T{{ N_param }}> Drop for ReadChunkIntoIter<'_, T{{ N_arg }}> {
...
impl<T{{ N_param }}> Iterator for ReadChunkIntoIter<'_, T{{ N_arg }}> {
    type Item = T;
...
impl<T{{ N_param }}> ExactSizeIterator for ReadChunkIntoIter<'_, T{{ N_arg }}> {}

impl<T{{ N_param }}> core::iter::FusedIterator for ReadChunkIntoIter<'_, T{{ N_arg }}> {}


#[cfg(feature = "std")]
impl{{ if_N("<const N: usize>") }} std::io::Write for Producer<{{ arc_tick_blank }}u8{{ N_arg }}> {
{% if contiguous %}
    #[inline]
...
#[cfg(feature = "std")]
impl{{ if_N("<const N: usize>") }} std::io::Read for Consumer<{{ arc_tick_blank }}u8{{ N_arg }}> {
{% if contiguous %}
    #[inline]
